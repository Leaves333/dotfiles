...
# add items to path:
# - cargo: for binaries installed through cargo
# - ghcup: for haskell install managed through ghcup
# - local: for ??? (codex on fedora desktop)
set -gx PATH $PATH ~/.cargo/bin/ ~/.ghcup/bin ~/.local/bin/

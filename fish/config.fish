...
set -gx EDITOR nvim
<<<<<<<<<<< conflict 1 of 2
+++++++++++ ssmzrxmn ece3d83e "desktop update"
<<<<<<< conflict 1 of 2
+++++++ ssmzrxmn 782682c9 "desktop update"
set -gx PATH $PATH ~/.cargo/bin/ ~/.local/bin/
%%%%%%% diff from: mrrzumou f3ffbbcb "update yazi icons and colors"
\\\\\\\        to: tnrknpxo a1953e9e "mechrevo update"
-set -gx PATH $PATH ~/.cargo/bin/
+set -gx PATH $PATH ~/.cargo/bin/ ~/.ghcup/bin
%%%%%%% diff from: yyxsswwy cb4058ec "update nvim plugins for 0.12"
\\\\\\\        to: muykuyty dfb4335a "thinkpad update"
-set -gx PATH $PATH ~/.cargo/bin/
...
<<<<<<< conflict 2 of 2
%%%%%%% diff from: mrrzumou f3ffbbcb "update yazi icons and colors"
\\\\\\\        to: ssmzrxmn 782682c9 "desktop update"
+
...
+  set -lx FZF_DEFAULT_OPTS_FILE
+
+  set -lx -- FZF_DEFAULT_COMMAND 'builtin history -z --show-time="%m-%d %H:%M%t"'
+
+  # Enable syntax highlighting colors on fish v4.3.3 and newer
+  if set -l -- v (string match -r -- '^(\d+)\.(\d+)(?:\.(\d+))?' $version)
+  and test "$v[2]" -gt 4 -o "$v[2]" -eq 4 -a \
...
+test -r '/home/leaves/.opam/opam-init/init.fish' && source '/home/leaves/.opam/opam-init/init.fish' > /dev/null 2> /dev/null; or true
+# END opam configuration
+++++++ muykuyty dfb4335a "thinkpad update"
